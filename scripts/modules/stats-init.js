...
function renderAllCharts() {
    ...
    // NOUVEAUX GRAPHIQUES
    if (typeof renderTUTByExerciseChart === 'function') {
        var exerciseData = sessions.flatMap(function(s) { return s.exercises || []; });
        renderTUTByExerciseChart(exerciseData);
    }
    if (typeof renderTempoDistributionChart === 'function') {
...
initNeonTracker3D({
score:Math.min(100,Math.round((completedCount/5)*50+(totalSets/60)*50)),
sessions:{current:completedCount,max:5},
sets:{current:totalSets,max:60}
});
}
