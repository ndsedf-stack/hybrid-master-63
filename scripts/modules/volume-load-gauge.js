...
export class VolumeLoadGauge {
  ...
  render() {
    ...
    const animate = (timestamp) => {
      ...
      // Mettre à jour les cartes individuellement par ID
      const setsEl = document.getElementById('setsValue');
      const tutEl = document.getElementById('tutValue');
      const volumeEl = document.getElementById('volumeValue');
      const statusEl = document.getElementById('statusValue');
      
      if (setsEl) setsEl.textContent = this.stats.setsCompleted;
      if (tutEl) {
        const minutes = Math.floor(this.stats.tutTotal / 60);
        const seconds = this.stats.tutTotal % 60;
        tutEl.textContent = minutes + ':' + String(seconds).padStart(2, '0');
      }
      if (volumeEl) volumeEl.textContent = (this.stats.weeklyVolume / 1000).toFixed(1) + 'k kg';
